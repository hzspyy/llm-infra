#!/usr/bin/env python3
"""Browser checks for the L5.10 delivery; use a dedicated browser session."""
import json
import os
...
OUT = ROOT / "outputs" / "lora-review-20260912-delivery"
...
BASE = "http://127.0.0.1:8735"
env = dict(os.environ, PATH=str(Path(NODE).parent) + ":" + os.environ["PATH"])
records = []
def call(*args):
    run = subprocess.run([NODE, CLI, "--session", "l510-review", *args], env=env,
...
def check_page():
    ...
    if "5.10" in result["title"]:
        assert result["svg"] == 2 and result["math"] >= 20, result
        assert all(s["shapes"] >= 9 and s["text"] >= 12 for s in result["svgChildren"]), result
...
try:
    ...
    call("snapshot", "-i")
    check_page()
    call("open", BASE + "/L5/5.10-lora-multitenant.html")
    call("snapshot", "-i")
    ...
    call("eval", "location.href=document.querySelector('a[href*=\"scheduler.py.html\"]').href; true")
    ...
    logs = decoded(call("eval", "JSON.stringify([...document.querySelectorAll('.srcref a')].filter(a=>/gpu-before|gpu-after|exit\\.txt/.test(a.href)).map(a=>({folded:!a.closest('details').open,lineNumbers:a.closest('.srcblock').querySelectorAll('.linenos').length,text:a.closest('.srcblock').querySelector('pre').textContent})))"))
    assert len(logs) == 3 and all(x["folded"] and x["lineNumbers"] == 0 for x in logs), logs
    call("eval", "const a=document.querySelector('a[href*=\"gpu-before.txt\"]'); a.closest('details').open=true; a.closest('details').scrollIntoView({block:'center'}); true")
    call("screenshot", str(OUT / "raw-log-light-834.png"))
    call("eval", "Promise.all([...document.querySelectorAll('a[href*=\"/raw/\"]')].map(async a=>{let r=await fetch(a.href,{method:'HEAD'});return {url:a.href,status:r.status}})).then(x=>JSON.stringify(x))")
finally:
    try:
        call("close")
    finally:
