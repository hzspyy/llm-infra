...
def instrument():
    ...
    def mapping(self,value):
        ...
        if TRACE and key not in SEEN and len(SEEN)<16:
            SEEN.add(key)
            emit('mapping',token_adapter_ids=list(value.index_mapping),
                 prompt_adapter_ids=list(value.prompt_mapping),gpu_slots=list(self.lora_index_to_id))
        return result
...
def run(root):
    ...
    llm=LLM(**cfg)
    reqs={i:LoRARequest(f'synthetic-{i}',i,str(root/'adapters'/f'adapter-{i}')) for i in range(1,6)}
    ...
    try:
        ...
        # Validate a rank mismatch in the real loader, then prove valid requests still run.
        try:
