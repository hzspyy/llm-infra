...
# ---------------------------------------------------------------- A
def part_a():
    """不加约束时，模型自己能吐出合法 JSON 吗？"""
    title("[A] 不约束时 JSON 的有效率")
    from vllm import SamplingParams
    llm = make_llm()
    n = 64
    sp = SamplingParams(temperature=0.8, top_p=0.95, max_tokens=96, seed=None)
    ...
    for o in outs:
        ...
        try:
            ...
            if (isinstance(obj, dict)
                ...
                bad_examples.append(("parse-ok/schema-bad", txt[:110]))
        except Exception:
            if len(bad_examples) < 3:
                bad_examples.append(("parse-fail", txt[:110]))
    print(f"\n  样本数              {n}")
    print(f"  json.loads 成功     {ok_parse}/{n}  ({100*ok_parse/n:.1f}%)")
    print(f"  且满足 schema       {ok_schema}/{n}  ({100*ok_schema/n:.1f}%)")
    if bad_examples:
        sub("失败样例（原文，未加工）")
        for kind, t in bad_examples:
            print(f"  [{kind}] {t!r}")
    shutdown(llm)
